...
const NewAccordion = () => {
  ...
  //    {
  //      question:
  //        "In React, what happens to a component when its state changes?",
  //      answer:
  //        "The component and all of its children are re-rendered to reflect the new state in the UI.",
  //    },
  //    {
  //      question:
  //        "What does the 'key' prop do when rendering the list of moves?",
  //      answer:
  //        "It provides a stable identity for each list item, helping React optimize re-renders and list updates.",
  //    },
  //    {
  ...
  return (
    <div>
      {data.map((el,i)=>{
        return (
          <div key={i}>
            <h1 onClick={() => handleClose(i)}>{el.question} </h1>
            {i == current && <p>{el.answer}</p>}
